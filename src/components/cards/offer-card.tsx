...
const STATIC_NOW_MS = 1735689600000; // 2025-01-01T00:00:00.000Z (mocked, to keep render pure)

export function OfferCard({ offer }: { offer: Offer }) {
  const daysAgo = Math.floor((STATIC_NOW_MS - new Date(offer.postedAt).getTime()) / (1000 * 60 * 60 * 24));
    ...
    <Card className="flex flex-col transition-shadow hover:shadow-md">
      ...
      <CardContent className="flex-1 pb-4">
        <div className="flex flex-wrap gap-3 mt-2 mb-4 text-xs text-slate-500">
          ...
          <div className="flex items-center gap-1">
            <Clock className="w-3.5 h-3.5" /> Il y a {daysAgo}j
          </div>
        </div>
